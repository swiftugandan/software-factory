#!/usr/bin/env bash
# Preflight: confirm the factory's dependencies are present. Run once after install.
# Usage: bash .claude/hooks/doctor.sh
ok=0
...
[ -x .claude/hooks/log-assumption.sh ] && echo "  ok   hooks are executable" || { echo "  MISS hooks not executable — run: chmod +x .claude/hooks/*.sh"; ok=1; }
